...
const slotCount = 100;
const imageBasePath = "assets/uploads/dress-slots";
...
async function saveCommitAndPush(payload) {
  ...
  const products = mergeWithFixedSlots(payload.products);
  ...
  await fsp.mkdir(path.join(root, "assets", "uploads", "dress-slots"), { recursive: true });
...
function safeUploadPath(value) {
  ...
  const targetPath = path.resolve(root, "assets", "uploads", relativePath);
...
function createDefaultProducts() {
  return Array.from({ length: slotCount }, (_, index) => {
    ...
    return {
      ...
      sizes: [],
      price: 0,
      frontImage: `${imageBasePath}/dress-${slot}-front.jpg`,
      backImage: `${imageBasePath}/dress-${slot}-back.jpg`,
      available: true,
...
function mergeWithFixedSlots(products) {
  ...
  return defaults.map((defaultProduct) => {
...
function normalizeProduct(product) {
  ...
  return {
    ...
    frontImage: String(product.frontImage || legacyImage || `${imageBasePath}/dress-${slot}-front.jpg`).trim(),
    backImage: String(product.backImage || `${imageBasePath}/dress-${slot}-back.jpg`).trim(),
    available: product.available !== false,
    featured: Boolean(product.featured),
    createdAt: product.createdAt || "2026-06-20",
